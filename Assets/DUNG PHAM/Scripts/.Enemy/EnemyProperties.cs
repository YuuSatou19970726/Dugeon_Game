...
public class EnemyProperties : MonoBehaviour
{
    ...
    [Header("Enemy Object")]
    [Range(0, 10000)] public float health;
    ...
    {
        health -= damage;
        StartCoroutine(BeKnockedBack());
        if (health <= 0) BeDeath();
    }
    IEnumerator BeKnockedBack()
    {
        yield return null;
        Vector3 playerPos = player.transform.position;
        Vector3 directionVector = transform.position - playerPos;
        rigid.AddForce(directionVector.normalized * speed * 10, ForceMode2D.Impulse);
        GameObject blood = Instantiate(bloodSplash, transform.position, Quaternion.identity);
        blood.transform.parent = transform;
    }
    public virtual void BeDeath()
    {
